...
const Projects = ({ posts }) => {
    ...
    useEffect(() => {
        const data = [];
        posts.map((item) => {
            const { fields: { projectType: { fields: { projectName } } } } = item
        ...
        <div className="flex flex-col items-center justify-center p-5 md:p-10">

            {
                types.map((name, index) => {
                    return <Grid container spacing={2} key={index} className="justify-centern">
                        <Typography textAlign="center" variant="h1" fontFamily="Hec" className=" text-black font-[900] text-3xl uppercase  w-full" >
                            {name}
                        </Typography>
                        {displayedPosts.filter((item) => item.fields.projectType.fields.projectName === name)
                            .map((item) => {
